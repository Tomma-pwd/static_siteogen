        ...
        if len(image_tuple) == 0:
            ...
            for image in image_tuple:
                ...
                if split_nodes_text[0] != '':
                    new_nodes.append(TextNode(split_nodes_text[0], TextNode.text_type_text))
                new_nodes.append(TextNode(image[0], TextNode.text_type_image, url = image[1]))
    ...
    for node in old_nodes:
        ...
        if len(link_tuple) == 0:
            ...
            for link in link_tuple:
                ...
                if split_nodes_text[0] != '':
                    new_nodes.append(TextNode(split_nodes_text[0], TextNode.text_type_text))
                new_nodes.append(TextNode(link[0], TextNode.text_type_image, url = link[1]))
                if split_nodes_text[1] != '':
                    new_nodes.append(TextNode(split_nodes_text[1], TextNode.text_type_text))
    return new_nodes
